...
OCCUP_COLUMNS = ("START", "LABEL", "TYPE", "FMTNAME")
MISC_COLUMNS = (
    "DEMONO",
    "DEMOCODE",
    "ENTRYDATE",
...
CISFMT_COLUMNS = ("START", "TYPE", "LABEL", "FMTNAME")
CHAR_LENGTHS = {
    "occup": {"START": 3, "LABEL": 25, "TYPE": 1, "FMTNAME": 8},
    "misc": {
        "DEMONO": 2,
        "DEMOCODE": 10,
        "ENTRYDATE": 7,
    ...
    "cisfmt": {"START": 10, "TYPE": 1, "LABEL": 100, "FMTNAME": 30},
}
...
def output_suffix(run_date: date) -> str:
    """Return YYMMDD suffix for the previous day's output files."""
...
def write_sas7bdat(
    ...
    """Write matching SAS7BDAT datasets using the configured SAS runtime."""
    try:
        import pandas as pd
        import saspy
    ...
    frames = {
        f"occup_{suffix}": ("occup", pd.DataFrame(occup_rows, columns=OCCUP_COLUMNS)),
        f"misc_{suffix}": ("misc", pd.DataFrame(misc_rows, columns=MISC_COLUMNS)),
        f"cisfmt_{suffix}": ("cisfmt", pd.DataFrame(cisfmt_rows, columns=CISFMT_COLUMNS)),
    }
    for _, (schema, frame) in frames.items():
        for column, length in CHAR_LENGTHS[schema].items():
            frame[column] = frame[column].fillna("").astype(str).str.slice(0, length)

    sas = saspy.SASsession()
    try:
        sas_path = str(output_dir).replace("'", "''")
        result = sas.submit(f"libname BKCTRL '{sas_path}';")
        ...
        for table, (schema, frame) in frames.items():
            sas.df2sd(
                ...
                table=table,
                libref="BKCTRL",
                char_lengths=CHAR_LENGTHS[schema],
            )
    finally:
        sas.endsas()

    destinations = {
        "occup_sas": output_dir / f"occup_{suffix}.sas7bdat",
        "misc_sas": output_dir / f"misc_{suffix}.sas7bdat",
        "cisfmt_sas": output_dir / f"cisfmt_{suffix}.sas7bdat",
    }
...
def write_outputs(
    ...
    """Write the converted BANK CONTROL data as text and SAS datasets."""
    output_dir.mkdir(parents=True, exist_ok=True)
    output_dir = output_dir.resolve()

    destinations = {
        "occup": output_dir / f"occup_{suffix}.txt",
        "misc": output_dir / f"misc_{suffix}.txt",
        "cisfmt": output_dir / f"cisfmt_{suffix}.txt",
    }
    write_txt(destinations["occup"], OCCUP_COLUMNS, occup_rows)
    write_txt(destinations["misc"], MISC_COLUMNS, misc_rows)
    write_txt(destinations["cisfmt"], CISFMT_COLUMNS, cisfmt_rows)
    destinations.update(write_sas7bdat(output_dir, suffix, occup_rows, misc_rows, cisfmt_rows))
    return destinations
...
def convert(
    ...
    return write_outputs(occup_rows, misc_rows, cisfmt_rows, output_dir, suffix)

"""Combined EIBMDPTR, EIBMDPFF, EIBMDPRC and EIBMDPPF report runner.

Reads the six DWH inputs once and shares staff exclusions/CIS deduplication.
Each named report applies its own selection, merge, aggregation and rendering.
EIBMDPRC requires character USER3 on SAVING, CURRENT and FD.

Keep combine2.py, PBBLNFMT.py, PBBDPFMT.py and PBMISFMT.py together.
...
Monthly execution on the 4th reports the previous month end:
    python combine2.py --run-date 2026-09-04
    python combine2.py --run-date 2026-09-04 --show-inputs
...
def prepare_common(saving, current, fd, cisca, cissa, cisfd):
    for name, df in [('SAVING',saving),('CURRENT',current),('FD',fd)]:
        require(df, ['ACCTNO','CURBAL','BRANCH','OPENIND','USER3', 'INTPLAN' if name=='FD' else 'PRODUCT'],name)
    ...
    # Keep source identity for the additional FD-only exclusions in EIBMDPPF.
    dp = pd.concat([
        saving.loc[~saving.PRODUCT.isin(STFSA)].assign(_SOURCE_DATASET='SAVING'),
        current.loc[~current.PRODUCT.isin(STFCA)].assign(_SOURCE_DATASET='CURRENT'),
        fd.loc[fd_mask].assign(_SOURCE_DATASET='FD'),
    ], ignore_index=True)
    dp = dp.loc[~dp.OPENIND.map(key).isin(['B','C','P'])].copy()
    return dp, cis


def build_report(dp, cis, report_name):
    """Apply report-specific filters before merging, as in each source job."""
    if report_name == 'EIBMDPTR':
        mask = dp.BRANCH.isin(BRANCHES)
    elif report_name == 'EIBMDPFF':
        mask = (dp.BRANCH > 1) & (dp.BRANCH < 271)
    elif report_name == 'EIBMDPPF':
        mask = (dp.BRANCH > 1) & (dp.BRANCH < 268)
        mask &= ~((dp._SOURCE_DATASET == 'FD') & dp.INTPLAN.between(429,431))
    elif report_name == 'EIBMDPRC':
        mask = dp.BRANCH.isin([66,168]) & dp.USER3.map(key).isin(['3','4','6','7','8'])
    else:
        raise ValueError(f'Unknown report: {report_name}')
    dp = dp.loc[mask].copy()
    dp['BRCHCD'] = 'XX' if report_name in ('EIBMDPFF','EIBMDPPF') else dp.BRANCH.map(
        lambda x: PBMISFMT.BRCHCD(x).rstrip()
    )
...
def render_EIBMDPTR(profile, reporting_date):
...
def render_EIBMDPFF(profile, reporting_date):
    """All-branch report: BRCHCD is an internal group and is not printed."""
    lines = [
        'PUBLIC BANK BERHAD',
        'DEVELOPMENT OF WEALTH MANAGEMENT CENTRES',
        'PROFILE FOR ALL BRANCHES -' + reporting_date.strftime('%d/%m/%y'),
        '',
        f'{"DEPOSIT RANGE":35} {"NO. OF CUSTOMER":>15} {"OUTSTANDING AMOUNT":>20}',
        '-' * 72,
    ]
    for row in profile.itertuples(index=False):
        lines.append(f'{row.DRANGE:35} {row.ACCT:15,.0f} {row.BALANCE:20,.2f}')
    if not profile.empty:
        lines.extend([
            '  ' + '-' * 67,
            f'{"      TOTAL":35} {profile.ACCT.sum():15,.0f} {profile.BALANCE.sum():20,.2f}',
            '  ' + '-' * 67,
        ])
    return '\n'.join(line.ljust(133) for line in lines) + '\n'


def render_EIBMDPRC(profile, reporting_date):
    lines = ['PUBLIC BANK BERHAD','DEVELOPMENT OF WEALTH MANAGEMENT CENTRES- RCB FOR KLC/BDA',
             'PROFILE FOR BDA (066) AND KLC (168) -'+reporting_date.strftime('%d/%m/%y'),'',
             f'{"BRH":5} {"DEPOSIT RANGE":35} {"NO. OF CUSTOMER":>15} {"OUTSTANDING AMOUNT":>20}', '-'*78]
    for branch, group in profile.groupby('BRCHCD',sort=True):
        for i, row in enumerate(group.itertuples(index=False)):
            lines.append(f'{branch if i==0 else "":5} {row.DRANGE:35} {row.ACCT:15,.0f} {row.BALANCE:20,.2f}')
...
def EIBMDPTR(dp, cis):
    return build_report(dp, cis, 'EIBMDPTR')


def EIBMDPFF(dp, cis):
    return build_report(dp, cis, 'EIBMDPFF')


def EIBMDPRC(dp, cis):
    return build_report(dp, cis, 'EIBMDPRC')


def EIBMDPPF(dp, cis):
    """Branches 2-267; FD INTPLAN 400-431 excluded; bank-wide customer grouping."""
    return build_report(dp, cis, 'EIBMDPPF')


def render_EIBMDPPF(profile, reporting_date):
    # The supplied SAS has the same all-branch report title and columns.
    return render_EIBMDPFF(profile, reporting_date)


REPORT_FLOWS = {
    'EIBMDPTR': (EIBMDPTR, render_EIBMDPTR, 'DPBR', 'DPBR_branch_totals'),
    'EIBMDPFF': (EIBMDPFF, render_EIBMDPFF, 'EIBMDPFF', 'EIBMDPFF_totals'),
    'EIBMDPRC': (EIBMDPRC, render_EIBMDPRC, 'EIBMDPRC', 'EIBMDPRC_branch_totals'),
    'EIBMDPPF': (EIBMDPPF, render_EIBMDPPF, 'EIBMDPPF', 'EIBMDPPF_totals'),
}


def run_reports(datasets):
    """Prepare once, then return independent profiles for all four flows."""
    dp, cis = prepare_common(**datasets)
    return {name: flow(dp, cis) for name, (flow, _, _, _) in REPORT_FLOWS.items()}


def write_reports(profiles, reporting_date, output_dir):
    """Write only the twelve report outputs; never write input datasets."""
    output_dir = Path(output_dir)
    output_dir.mkdir(parents=True, exist_ok=True)
    for name, profile in profiles.items():
        _, render, stem, total_stem = REPORT_FLOWS[name]
        profile.to_csv(output_dir / f'{stem}.csv', index=False, float_format='%.2f')
        totals = profile.groupby('BRCHCD')[['ACCT','BALANCE']].sum().reset_index()
        totals.to_csv(output_dir / f'{total_stem}.csv', index=False, float_format='%.2f')
        (output_dir / f'{stem}_report.txt').write_text(render(profile, reporting_date), encoding='utf-8')
        print(f'{name}: {len(profile)} profile rows; output {output_dir / stem}')
...
def main():
    ...
    # Build all profiles successfully before writing report files.
    profiles = run_reports(datasets)
    write_reports(profiles, reporting_date, args.output_dir)

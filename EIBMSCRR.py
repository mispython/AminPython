"""EIBMSCRR: individual job; all input paths must be .sas7bdat.
...
def transform(srs, branches=None):
    ...
    return nway(srs,['REGNID', 'BRCHCD', 'STAFF', 'PRODUCT'],METRICS)
...
def main():
    ...
    lines=category_report(result,'EIBMSCRR',dt,['REGNID', 'BRCHCD'],product_counts=False,source_grand_bug=False)
    args.output_dir.mkdir(parents=True,exist_ok=True)
    dump(result,args.output_dir/'EIBMSCRR.csv')
    save_report(lines,args.output_dir/'EIBMSCRR_report.txt')
